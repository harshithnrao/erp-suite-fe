...
import { PermissionService } from "src/app/permission.service";import { Proposal } from "./proposal-create-update/proposal.model";
import { ProposalCreateUpdateComponent } from "./proposal-create-update/proposal-create-update.component";
...
export class ProposalComponent implements OnInit, AfterViewInit,OnDestroy {

  crumbs = [
    { label: 'CRM', url: '/crm' },
  ];

  subject$: ReplaySubject<Proposal[]> = new ReplaySubject<Proposal[]>(1);
  data$: Observable<Proposal[]> = this.subject$.asObservable();
  quotes: Proposal[];

  quoteResponseData: Proposal[] = [];
  ...
  columns: ListColumn[] = [
    ...
    { name: 'Proposal Text', property: 'text', visible: true, isModelProperty: true },
    { name: 'Total Amount', property: 'totalAmount', visible: true, isModelProperty: true },
    { name: 'Proposal Date', property: 'proposalDate', visible: true, isModelProperty: true },
  ...
  constructor(private dialog: MatDialog, private quoteService: ProposalService, private commonService: CommonService,protected permissionService: PermissionService) {
  }
  ...
  async getData() {
    this.isLoaded = false;
    const response: Proposal[] = await this.quoteService.getQuotes();
    this.isLoaded = true;
    of(response.map(quote => new Proposal(quote))).subscribe(quotes => {
  ...
  createQuote() {
    this.dialog.open(ProposalCreateUpdateComponent).afterClosed().subscribe(async (quote: Proposal) => {
      /**
      ...
      if (quote) {
        ...
        if (response && response.data && response.data.id) {
          this.quotes.unshift(new Proposal(quote));
  ...
  updateQuote(quote) {
    this.dialog.open(ProposalCreateUpdateComponent, {
      data: quote
      ...
      if (quote) {
        ...
        if (response && response.data && response.data.id) {
          ...
          this.quotes[index] = new Proposal(quote);
          this.subject$.next(this.quotes);
