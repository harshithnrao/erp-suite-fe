...
import { PermissionService } from "src/app/permission.service";import { Log } from "./log-create-update/log.model";
import { LogCreateUpdateComponent } from "./log-create-update/log-create-update.component";
...
export class LogComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  subject$: ReplaySubject<Log[]> = new ReplaySubject<Log[]>(1);
  data$: Observable<Log[]> = this.subject$.asObservable();
  quotes: Log[];

  quoteResponseData: Log[] = [];
  ...
  columns: ListColumn[] = [
    ...
    { name: 'Task', property: 'task', visible: false, isModelProperty: true },
  ...
  pageSize = 10;
  dataSource: MatTableDataSource<Log> | null;

  @ViewChild(MatPaginator, { static: true }) paginator: MatPaginator;
  @ViewChild(MatSort, { static: true }) sort: MatSort;
  isLoaded: boolean = false;

  constructor(private dialog: MatDialog, private quoteService: LogService, private commonService: CommonService,protected permissionService: PermissionService) {
  }
  ...
  async getData() {
    this.isLoaded = false;
    const response: Log[] = await this.quoteService.getQuotes();
    this.isLoaded = true;
    of(response.map(quote => new Log(quote))).subscribe(quotes => {
  ...
  createQuote() {
    this.dialog.open(LogCreateUpdateComponent).afterClosed().subscribe(async (quote: Log) => {
      /**
      ...
      if (quote) {
        ...
        if (response && response.data && response.data.id) {
          this.quotes.unshift(new Log(quote));
  ...
  updateQuote(quote) {
    this.dialog.open(LogCreateUpdateComponent, {
      data: quote
      ...
      if (quote) {
        ...
        if (response && response.data && response.data.id) {
          ...
          this.quotes[index] = new Log(quote);
          this.subject$.next(this.quotes);
