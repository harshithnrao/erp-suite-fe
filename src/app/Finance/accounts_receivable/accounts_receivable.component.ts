...
import { PermissionService } from "src/app/permission.service";import { AccountsReceivable } from "./accounts_receivable-create-update/accounts_receivable.model";
import { AccountsReceivableCreateUpdateComponent } from "./accounts_receivable-create-update/accounts_receivable-create-update.component";
import { AccountsReceivableService } from "./accounts_receivable.service";

@Component({
  selector: 'fury-users-list',
  templateUrl: './accounts_receivable.component.html',
  styleUrls: ['./accounts_receivable.component.scss'],
  animations: [fadeInRightAnimation, fadeInUpAnimation]
})
export class AccountsReceivableComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  quotes: AccountsReceivable[];

  quoteResponseData: AccountsReceivable[] = [];
  ...
  constructor(private dialog: MatDialog, private quoteService: AccountsReceivableService, private commonService: CommonService,protected permissionService: PermissionService) {
  }
  ...
  async getData() {
    this.isLoaded = false;
    const response: AccountsReceivable[] = await this.quoteService.getQuotes();
    this.isLoaded = true;
    of(response.map(quote => new AccountsReceivable(quote))).subscribe(quotes => {
  ...
  createQuote() {
    this.dialog.open(AccountsReceivableCreateUpdateComponent).afterClosed().subscribe(async (quote: AccountsReceivable) => {
      /**
      ...
      if (quote) {
        ...
        if (response && response.data && response.data.id) {
          this.quotes.unshift(new AccountsReceivable(quote));
    ...
    this.dialog.open(AccountsReceivableCreateUpdateComponent, {
      ...
      if (quote) {
        ...
        if (response && response.data && response.data.id) {
          ...
          this.quotes[index] = new AccountsReceivable(quote);
          this.subject$.next(this.quotes);
