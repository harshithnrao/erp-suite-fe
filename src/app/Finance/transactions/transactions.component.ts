...
import { PermissionService } from "src/app/permission.service";import { Transactions } from "./transactions-create-update/transactions.model";
...
export class TransactionsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  subject$: ReplaySubject<Transactions[]> = new ReplaySubject<Transactions[]>(1);
  data$: Observable<Transactions[]> = this.subject$.asObservable();
  quotes: Transactions[];

  quoteResponseData: Transactions[] = [];

  @Input()
  columns: ListColumn[] = [
    { name: 'Checkbox', property: 'checkbox', visible: false },
    { name: 'ID', property: 'id', visible: true, isModelProperty: true },
    { name: 'AR Account ID', property: 'arAccountId', visible: true, isModelProperty: true },
  ...
  pageSize = 10;
  dataSource: MatTableDataSource<Transactions> | null;

  @ViewChild(MatPaginator, { static: true }) paginator: MatPaginator;
  @ViewChild(MatSort, { static: true }) sort: MatSort;
  isLoaded: boolean = false;

  constructor(private dialog: MatDialog, private quoteService: TransactionsService, private commonService: CommonService,protected permissionService: PermissionService) {
  }
  ...
  async getData() {
    this.isLoaded = false;
    const response: Transactions[] = await this.quoteService.getQuotes();
    this.isLoaded = true;
    of(response.map(quote => new Transactions(quote))).subscribe(quotes => {
  ...
  createQuote() {
    this.dialog.open(TransactionsCreateUpdateComponent).afterClosed().subscribe(async (quote: Transactions) => {
      /**
      ...
      if (quote) {
        ...
        if (response && response.data && response.data.id) {
          this.quotes.unshift(new Transactions(quote));
  ...
  updateQuote(quote) {
    this.dialog.open(TransactionsCreateUpdateComponent, {
      data: quote
      ...
      if (quote) {
        ...
        if (response && response.data && response.data.id) {
          ...
          this.quotes[index] = new Transactions(quote);
          this.subject$.next(this.quotes);
