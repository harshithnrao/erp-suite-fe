...
import { PermissionService } from "src/app/permission.service";import { APPOTransactions } from "./APPOtransactions-create-update/APPOtransactions.model";
import { APPOTransactionsCreateUpdateComponent } from "./APPOtransactions-create-update/APPOtransactions-create-update.component";
import { APPOTransactionsService } from "./APPOtransactions.service";

@Component({
  selector: 'fury-users-list',
  templateUrl: './APPOtransactions.component.html',
  styleUrls: ['./APPOtransactions.component.scss'],
  animations: [fadeInRightAnimation, fadeInUpAnimation]
})
export class APPOTransactionsComponent implements OnInit, AfterViewInit, OnDestroy {

  crumbs = [
    { label: 'Finance', url: '/finance' },
  ];

  subject$: ReplaySubject<APPOTransactions[]> = new ReplaySubject<APPOTransactions[]>(1);
  data$: Observable<APPOTransactions[]> = this.subject$.asObservable();
  quotes: APPOTransactions[];

  quoteResponseData: APPOTransactions[] = [];
  ...
  columns: ListColumn[] = [
    ...
    { name: 'AP Account ID', property: 'apAccountId', visible: true, isModelProperty: true },
    { name: 'To Account', property: 'toAccount', visible: true, isModelProperty: true },
    { name: 'Purchase Order ID', property: 'purchaseOrderId', visible: true, isModelProperty: true },
  ...
  pageSize = 10;
  dataSource: MatTableDataSource<APPOTransactions> | null;

  @ViewChild(MatPaginator, { static: true }) paginator: MatPaginator;
  @ViewChild(MatSort, { static: true }) sort: MatSort;
  isLoaded: boolean = false;

  constructor(private dialog: MatDialog, private quoteService: APPOTransactionsService, private commonService: CommonService,protected permissionService: PermissionService) {
  }
  ...
  async getData() {
    this.isLoaded = false;
    const response: APPOTransactions[] = await this.quoteService.getQuotes();
    this.isLoaded = true;
    of(response.map(quote => new APPOTransactions(quote))).subscribe(quotes => {
  ...
  createQuote() {
    this.dialog.open(APPOTransactionsCreateUpdateComponent).afterClosed().subscribe(async (quote: APPOTransactions) => {
      /**
      ...
      if (quote) {
        ...
        if (response && response.data && response.data.id) {
          this.quotes.unshift(new APPOTransactions(quote));
  ...
  updateQuote(quote) {
    this.dialog.open(APPOTransactionsCreateUpdateComponent, {
      data: quote
      ...
      if (quote) {
        ...
        if (response && response.data && response.data.id) {
          ...
          this.quotes[index] = new APPOTransactions(quote);
          this.subject$.next(this.quotes);
