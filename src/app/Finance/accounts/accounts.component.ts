...
import { PermissionService } from "src/app/permission.service";import { Accounts } from "./accounts-create-update/accounts.model";
...
export class AccountsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  subject$: ReplaySubject<Accounts[]> = new ReplaySubject<Accounts[]>(1);
  data$: Observable<Accounts[]> = this.subject$.asObservable();
  quotes: Accounts[];

  quoteResponseData: Accounts[] = [];
  ...
  pageSize = 10;
  dataSource: MatTableDataSource<Accounts> | null;

  @ViewChild(MatPaginator, { static: true }) paginator: MatPaginator;
  @ViewChild(MatSort, { static: true }) sort: MatSort;
  isLoaded: boolean = false;

  constructor(private dialog: MatDialog, private quoteService: AccountsService, private commonService: CommonService,protected permissionService: PermissionService) {
  }
  ...
  async getData() {
    this.isLoaded = false;
    const response: Accounts[] = await this.quoteService.getQuotes();
    this.isLoaded = true;
    of(response.map(quote => new Accounts(quote))).subscribe(quotes => {
  ...
  createQuote() {
    this.dialog.open(AccountsCreateUpdateComponent).afterClosed().subscribe(async (quote: Accounts) => {
      /**
      ...
      if (quote) {
        ...
        if (response && response.data && response.data.id) {
          this.quotes.unshift(new Accounts(quote));
    ...
    this.dialog.open(AccountsCreateUpdateComponent, {
      ...
      if (quote) {
        ...
        if (response && response.data && response.data.id) {
          ...
          this.quotes[index] = new Accounts(quote);
          this.subject$.next(this.quotes);
