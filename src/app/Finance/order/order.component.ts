...
import { PermissionService } from "src/app/permission.service";import { Order } from "./order-create-update/order.model";
...
export class OrderComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  columns: ListColumn[] = [
    ...
    { name: 'Proposed Amount', property: 'proposedAmount', visible: true, isModelProperty: true },
    { name: 'Item', property: 'item', visible: true, isModelProperty: true },
    { name: 'Item Count', property: 'itemCount', visible: false, isModelProperty: true },
  ...
  constructor(private dialog: MatDialog, private quoteService: OrderService, private commonService: CommonService,protected permissionService: PermissionService) {
  }
  ...
  createQuote() {
    this.dialog.open(OrderCreateUpdateComponent).afterClosed().subscribe(async (quote: Order) => {
      /**
