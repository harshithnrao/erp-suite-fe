...
import { PermissionService } from "src/app/permission.service";import { APSOTransactions } from "./APSOtransactions-create-update/APSOtransactions.model";
import { APSOTransactionsCreateUpdateComponent } from "./APSOtransactions-create-update/APSOtransactions-create-update.component";
import { APSOTransactionsService } from "./APSOtransactions.service";

@Component({
  selector: 'fury-users-list',
  templateUrl: './APSOtransactions.component.html',
  styleUrls: ['./APSOtransactions.component.scss'],
  animations: [fadeInRightAnimation, fadeInUpAnimation]
})
export class APSOTransactionsComponent implements OnInit, AfterViewInit, OnDestroy {

  crumbs = [
    { label: 'Finance', url: '/finance' },
  ];

  subject$: ReplaySubject<APSOTransactions[]> = new ReplaySubject<APSOTransactions[]>(1);
  data$: Observable<APSOTransactions[]> = this.subject$.asObservable();
  quotes: APSOTransactions[];

  quoteResponseData: APSOTransactions[] = [];
  ...
  columns: ListColumn[] = [
    ...
    { name: 'Service Order ID', property: 'serviceOrderId', visible: true, isModelProperty: true },
  ...
  pageSize = 10;
  dataSource: MatTableDataSource<APSOTransactions> | null;

  @ViewChild(MatPaginator, { static: true }) paginator: MatPaginator;
  @ViewChild(MatSort, { static: true }) sort: MatSort;
  isLoaded: boolean = false;

  constructor(private dialog: MatDialog, private quoteService: APSOTransactionsService, private commonService: CommonService,protected permissionService: PermissionService) {
  }
  ...
  async getData() {
    this.isLoaded = false;
    const response: APSOTransactions[] = await this.quoteService.getQuotes();
    this.isLoaded = true;
    of(response.map(quote => new APSOTransactions(quote))).subscribe(quotes => {
  ...
  createQuote() {
    this.dialog.open(APSOTransactionsCreateUpdateComponent).afterClosed().subscribe(async (quote: APSOTransactions) => {
      /**
      ...
      if (quote) {
        ...
        if (response && response.data && response.data.id) {
          this.quotes.unshift(new APSOTransactions(quote));
  ...
  updateQuote(quote) {
    this.dialog.open(APSOTransactionsCreateUpdateComponent, {
      data: quote
      ...
      if (quote) {
        ...
        if (response && response.data && response.data.id) {
          ...
          this.quotes[index] = new APSOTransactions(quote);
          this.subject$.next(this.quotes);
