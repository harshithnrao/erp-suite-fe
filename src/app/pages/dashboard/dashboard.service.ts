...
import { MenuItem } from 'src/app/HRMS/hrms.service';
...
export class DashboardService {
  ...
  getAdvancedPieChartData() {
    const labels: string[] = [];
    const data: number[] = [];
    if (this.screenModel.categories && this.screenModel.categories.length > 0) {
      this.screenModel.categories.forEach(category => {
        labels.push(category.order_status.toUpperCase());
        data.push(parseInt(category.order_status_count));
      });
    }
    return of({
      labels: labels,
  ...
  getSalesSummary() {
    this.screenModel.salesSummary = {
      previousMonth: 0,
      thisMonth: 0
    }
    if (this.screenModel.quotesSummary && this.screenModel.quotesSummary.length > 0) {
      this.screenModel.salesSummary = {
        previousMonth: this.screenModel.quotesSummary[0].lastmonth,
        thisMonth: this.screenModel.quotesSummary[0].currmonth
      }
    }
  ...
  private jsonUrl = 'assets/homescreen/homescreen.config.json';//src\assets\finance\menu-items.finance.json
  ...
  async getDashboardData() {
    const categoryRequest: HttpRequestModel = {
      url: Utility.urlParams.getCategories.url,
      param: {},
      type: Utility.urlParams.getCategories.type
    };

    const quotesSummary: HttpRequestModel = {
      url: Utility.urlParams.getQuotesSummary.url,
      param: {},
      type: Utility.urlParams.getQuotesSummary.type
    };

    const customersCount: HttpRequestModel = {
      url: Utility.urlParams.getCustomersCount.url,
      param: {},
      type: Utility.urlParams.getCustomersCount.type
    };
    // const reqArr: HttpRequestModel[] = [categoryRequest, quotesSummary, customersCount];
    // const responses = await lastValueFrom(this.commonService.getGenericForkJoin(reqArr));
    // if (responses) {
    //   this.screenModel.categories = responses[0];
    //   this.screenModel.quotesSummary = responses[1];
    //   this.screenModel.customersCount = responses[2];
