...
import { PermissionService } from "src/app/permission.service";import { Leaves } from "./leaves-create-update/leaves.model";
import { LeavesCreateUpdateComponent } from "./leaves-create-update/leaves-create-update.component";
...
export class LeavesComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  subject$: ReplaySubject<Leaves[]> = new ReplaySubject<Leaves[]>(1);
  data$: Observable<Leaves[]> = this.subject$.asObservable();
  quotes: Leaves[];

  quoteResponseData: Leaves[] = [];
  ...
  columns: ListColumn[] = [
    ...
    { name: 'Leave Type ID', property: 'leaveTypeId', visible: false, isModelProperty: true },
    { name: 'Start Date', property: 'startDate', visible: true, isModelProperty: true },
    { name: 'End Date', property: 'endDate', visible: true, isModelProperty: true },
    ...
    // Adding Actions field at the end
    { name: 'Actions', property: 'actions', visible: true, isModelProperty: false }


  ] as ListColumn[];
  pageSize = 10;
  dataSource: MatTableDataSource<Leaves> | null;

  @ViewChild(MatPaginator, { static: true }) paginator: MatPaginator;
  @ViewChild(MatSort, { static: true }) sort: MatSort;
  isLoaded: boolean = false;

  constructor(private dialog: MatDialog, private quoteService: LeavesService, private commonService: CommonService,protected permissionService: PermissionService) {
  }
  ...
  async getData() {
    this.isLoaded = false;
    const response: Leaves[] = await this.quoteService.getQuotes();
    this.isLoaded = true;
    of(response.map(quote => new Leaves(quote))).subscribe(quotes => {
  ...
  createQuote() {
    this.dialog.open(LeavesCreateUpdateComponent).afterClosed().subscribe(async (quote: Leaves) => {
      /**
      ...
      if (quote) {
        ...
        if (response && response.data && response.data.id) {
          this.quotes.unshift(new Leaves(quote));
  ...
  updateQuote(quote) {
    this.dialog.open(LeavesCreateUpdateComponent, {
      data: quote
      ...
      if (quote) {
        ...
        if (response && response.data && response.data.id) {
          ...
          this.quotes[index] = new Leaves(quote);
          this.subject$.next(this.quotes);
