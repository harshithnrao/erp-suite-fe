...
import { PermissionService } from "src/app/permission.service";import { PO } from "./po-create-update/po.model";
import { POCreateUpdateComponent } from "./po-create-update/po-create-update.component";
...
export class POComponent implements OnInit, AfterViewInit, OnDestroy {

  crumbs = [
    { label: 'SCM', url: '/scm' },
  ];

  subject$: ReplaySubject<PO[]> = new ReplaySubject<PO[]>(1);
  data$: Observable<PO[]> = this.subject$.asObservable();
  quotes: PO[];

  quoteResponseData: PO[] = [];
  ...
  columns: ListColumn[] = [
    ...
    { name: 'Item Count', property: 'itemCount', visible: true, isModelProperty: true },
  ...
  pageSize = 10;
  dataSource: MatTableDataSource<PO> | null;

  @ViewChild(MatPaginator, { static: true }) paginator: MatPaginator;
  @ViewChild(MatSort, { static: true }) sort: MatSort;
  isLoaded: boolean = false;

  constructor(private dialog: MatDialog, private quoteService: POService, private commonService: CommonService,protected permissionService: PermissionService) {
  }
  ...
  async getData() {
    this.isLoaded = false;
    const response: PO[] = await this.quoteService.getQuotes();
    this.isLoaded = true;
    of(response.map(quote => new PO(quote))).subscribe(quotes => {
  ...
  createQuote() {
    this.dialog.open(POCreateUpdateComponent).afterClosed().subscribe(async (quote: PO) => {
      /**
      ...
      if (quote) {
        ...
        if (response && response.data && response.data.id) {
          this.quotes.unshift(new PO(quote));
  ...
  updateQuote(quote) {
    this.dialog.open(POCreateUpdateComponent, {
      data: quote
      ...
      if (quote) {
        ...
        if (response && response.data && response.data.id) {
          ...
          this.quotes[index] = new PO(quote);
          this.subject$.next(this.quotes);
