...
import { PermissionService } from "src/app/permission.service";import { DistributionCenter } from "./distribution-center-create-update/distribution-center.model";
import { DistributionCenterCreateUpdateComponent } from "./distribution-center-create-update/distribution-center-create-update.component";
...
export class DistributionCenterComponent implements OnInit, AfterViewInit, OnDestroy {

  crumbs = [
    { label: 'SCM', url: '/scm' },
  ];

  subject$: ReplaySubject<DistributionCenter[]> = new ReplaySubject<DistributionCenter[]>(1);
  data$: Observable<DistributionCenter[]> = this.subject$.asObservable();
  quotes: DistributionCenter[];

  quoteResponseData: DistributionCenter[] = [];
  ...
  columns: ListColumn[] = [
    ...
    { name: 'Capacity', property: 'capacity', visible: true, isModelProperty: true },
  ...
  pageSize = 10;
  dataSource: MatTableDataSource<DistributionCenter> | null;

  @ViewChild(MatPaginator, { static: true }) paginator: MatPaginator;
  @ViewChild(MatSort, { static: true }) sort: MatSort;
  isLoaded: boolean = false;

  constructor(private dialog: MatDialog, private quoteService: DistributionCenterService, private commonService: CommonService,protected permissionService: PermissionService) {
  }
  ...
  async getData() {
    this.isLoaded = false;
    const response: DistributionCenter[] = await this.quoteService.getQuotes();
    this.isLoaded = true;
    of(response.map(quote => new DistributionCenter(quote))).subscribe(quotes => {
  ...
  createQuote() {
    this.dialog.open(DistributionCenterCreateUpdateComponent).afterClosed().subscribe(async (quote: DistributionCenter) => {
      /**
      ...
      if (quote) {
        ...
        if (response && response.data && response.data.id) {
          this.quotes.unshift(new DistributionCenter(quote));
  ...
  updateQuote(quote) {
    this.dialog.open(DistributionCenterCreateUpdateComponent, {
      data: quote
      ...
      if (quote) {
        ...
        if (response && response.data && response.data.id) {
          ...
          this.quotes[index] = new DistributionCenter(quote);
          this.subject$.next(this.quotes);
