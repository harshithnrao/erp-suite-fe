...
import { PermissionService } from "src/app/permission.service";import { Customer } from "./customer-create-update/customer.model";
...
export class CustomerComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(private dialog: MatDialog, private quoteService: CustomerService, private commonService: CommonService,protected permissionService: PermissionService) {
  }
  ...
  async getData() {
    this.isLoaded = false;
    const response: Customer[] = await this.quoteService.getQuotes();
    this.isLoaded = true;
    of(response.map(quote => new Customer(quote))).subscribe(quotes => {
  ...
  createQuote() {
    this.dialog.open(CustomerCreateUpdateComponent).afterClosed().subscribe(async (quote: Customer) => {
      /**
      ...
      if (quote) {
        ...
        if (response && response.data && response.data.id) {
          this.quotes.unshift(new Customer(quote));
    ...
    this.dialog.open(CustomerCreateUpdateComponent, {
      ...
      if (quote) {
        ...
        if (response && response.data && response.data.id) {
          ...
          this.quotes[index] = new Customer(quote);
          this.subject$.next(this.quotes);
