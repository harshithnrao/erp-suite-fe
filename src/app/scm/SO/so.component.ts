...
import { PermissionService } from "src/app/permission.service"; import { SO } from "./so-create-update/so.model";
...
export class SOComponent implements OnInit, AfterViewInit, OnDestroy {

  crumbs = [
    { label: 'SCM', url: '/scm' },
  ];

  subject$: ReplaySubject<SO[]> = new ReplaySubject<SO[]>(1);
  data$: Observable<SO[]> = this.subject$.asObservable();
  quotes: SO[];

  quoteResponseData: SO[] = [];
  ...
  columns: ListColumn[] = [
    ...
    { name: 'Service Order Type ID', property: 'serviceOrderTypeId', visible: true, isModelProperty: true },
    { name: 'Service Order Type Name', property: 'serviceOrderTypeName', visible: true, isModelProperty: true },
    { name: 'Order Date', property: 'orderDate', visible: true, isModelProperty: true },
  ...
  pageSize = 10;
  dataSource: MatTableDataSource<SO> | null;

  @ViewChild(MatPaginator, { static: true }) paginator: MatPaginator;
  @ViewChild(MatSort, { static: true }) sort: MatSort;
  isLoaded: boolean = false;

  constructor(private dialog: MatDialog, private quoteService: SOService, private commonService: CommonService, protected permissionService: PermissionService) {
  }
  ...
  async getData() {
    this.isLoaded = false;
    const response: SO[] = await this.quoteService.getQuotes();
    this.isLoaded = true;
    of(response.map(quote => new SO(quote))).subscribe(quotes => {
  ...
  createQuote() {
    this.dialog.open(SOCreateUpdateComponent).afterClosed().subscribe(async (quote: SO) => {
      /**
      ...
      if (quote) {
        ...
        if (response && response.data && response.data.id) {
          this.quotes.unshift(new SO(quote));
  ...
  updateQuote(quote) {
    this.dialog.open(SOCreateUpdateComponent, {
      data: quote
      ...
      if (quote) {
        ...
        if (response && response.data && response.data.id) {
          ...
          this.quotes[index] = new SO(quote);
          this.subject$.next(this.quotes);
