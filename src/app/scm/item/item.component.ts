...
import { PermissionService } from "src/app/permission.service";import { Item } from "./item-create-update/item.model";
...
export class ItemComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  quoteResponseData: Item[] = [];

@Input()
columns: ListColumn[] = [
  { name: 'Checkbox', property: 'checkbox', visible: false },
  { name: 'ID', property: 'id', visible: true, isModelProperty: true },
  { name: 'Category Code', property: 'categoryCode', visible: true, isModelProperty: true },
  { name: 'Category Name', property: 'categoryName', visible: true, isModelProperty: true },
  ...
  constructor(private dialog: MatDialog, private quoteService: ItemService, private commonService: CommonService,protected permissionService: PermissionService) {
  }
  ...
  async getData() {
    this.isLoaded = false;
    const response: Item[] = await this.quoteService.getQuotes();
    this.isLoaded = true;
    of(response.map(quote => new Item(quote))).subscribe(quotes => {
  ...
  createQuote() {
    this.dialog.open(ItemCreateUpdateComponent).afterClosed().subscribe(async (quote: Item) => {
      /**
      ...
      if (quote) {
        ...
        if (response && response.data && response.data.id) {
          this.quotes.unshift(new Item(quote));
    ...
    this.dialog.open(ItemCreateUpdateComponent, {
      ...
      if (quote) {
        ...
        if (response && response.data && response.data.id) {
          ...
          this.quotes[index] = new Item(quote);
          this.subject$.next(this.quotes);
