...
import { PermissionService } from "src/app/permission.service";import { Category } from "./category-create-update/category.model";
...
export class CategoryComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  columns: ListColumn[] = [
    ...
    { name: 'Valid From', property: 'validFrom', visible: true, isModelProperty: true },
    { name: 'Valid Till', property: 'validTill', visible: true, isModelProperty: true },
  ...
  constructor(private dialog: MatDialog, private quoteService: CategoryService, private commonService: CommonService,protected permissionService: PermissionService) {
  }
  ...
  async getData() {
    this.isLoaded = false;
    const response: Category[] = await this.quoteService.getQuotes();
    this.isLoaded = true;
    of(response.map(quote => new Category(quote))).subscribe(quotes => {
  ...
  createQuote() {
    this.dialog.open(CategoryCreateUpdateComponent).afterClosed().subscribe(async (quote: Category) => {
      /**
      ...
      if (quote) {
        ...
        if (response && response.data && response.data.id) {
          this.quotes.unshift(new Category(quote));
    ...
    this.dialog.open(CategoryCreateUpdateComponent, {
      ...
      if (quote) {
        ...
        if (response && response.data && response.data.id) {
          ...
          this.quotes[index] = new Category(quote);
          this.subject$.next(this.quotes);
