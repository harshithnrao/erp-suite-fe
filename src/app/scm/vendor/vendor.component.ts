...
import { PermissionService } from "src/app/permission.service";import { Vendor } from "./vendor-create-update/vendor.model";
...
export class VendorComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  subject$: ReplaySubject<Vendor[]> = new ReplaySubject<Vendor[]>(1);
  data$: Observable<Vendor[]> = this.subject$.asObservable();
  quotes: Vendor[];

  quoteResponseData: Vendor[] = [];
  ...
  columns: ListColumn[] = [
    ...
    { name: 'Point of Contact', property: 'pointOfContact', visible: true, isModelProperty: true },
    { name: 'Contact Info', property: 'contactInfo', visible: true, isModelProperty: true },
  ...
  constructor(private dialog: MatDialog, private quoteService: VendorService, private commonService: CommonService,protected permissionService: PermissionService) {
  }
  ...
  async getData() {
    this.isLoaded = false;
    const response: Vendor[] = await this.quoteService.getQuotes();
    this.isLoaded = true;
    of(response.map(quote => new Vendor(quote))).subscribe(quotes => {
  ...
  createQuote() {
    this.dialog.open(VendorCreateUpdateComponent).afterClosed().subscribe(async (quote: Vendor) => {
      /**
      ...
      if (quote) {
        ...
        if (response && response.data && response.data.id) {
          this.quotes.unshift(new Vendor(quote));
    ...
    this.dialog.open(VendorCreateUpdateComponent, {
      ...
      if (quote) {
        ...
        if (response && response.data && response.data.id) {
          ...
          this.quotes[index] = new Vendor(quote);
          this.subject$.next(this.quotes);
