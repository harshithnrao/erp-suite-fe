...
import { PermissionService } from "src/app/permission.service";import { Zone } from "./zone-create-update/zone.model";
import { ZoneCreateUpdateComponent } from "./zone-create-update/zone-create-update.component";
...
export class ZoneComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  subject$: ReplaySubject<Zone[]> = new ReplaySubject<Zone[]>(1);
  data$: Observable<Zone[]> = this.subject$.asObservable();
  quotes: Zone[];

  quoteResponseData: Zone[] = [];
  ...
  columns: ListColumn[] = [
    ...
    { name: 'Pincode', property: 'pincode', visible: true, isModelProperty: true },
  ...
  constructor(private dialog: MatDialog, private quoteService: ZoneService, private commonService: CommonService,protected permissionService: PermissionService) {
  }
  ...
  async getData() {
    this.isLoaded = false;
    const response: Zone[] = await this.quoteService.getQuotes();
    this.isLoaded = true;
    of(response.map(quote => new Zone(quote))).subscribe(quotes => {
  ...
  createQuote() {
    this.dialog.open(ZoneCreateUpdateComponent).afterClosed().subscribe(async (quote: Zone) => {
      /**
      ...
      if (quote) {
        ...
        if (response && response.data && response.data.id) {
          this.quotes.unshift(new Zone(quote));
  ...
  updateQuote(quote) {
    this.dialog.open(ZoneCreateUpdateComponent, {
      data: quote
      ...
      if (quote) {
        ...
        if (response && response.data && response.data.id) {
          ...
          this.quotes[index] = new Zone(quote);
          this.subject$.next(this.quotes);
